...
namespace TheVegetationEngineBlanket
{
    public class TVETSHub : EditorWindow
    {
        float GUI_HALF_EDITOR_WIDTH = 220;

        string autorunPath;
        string assetFolder = "Assets/BOXOPHOBIC/The Vegetation Engine Modules/Terrain Shaders";
        ...
        static TVETSHub window;

        //Vector2 scrollPosition = Vector2.zero;

        [MenuItem("Window/BOXOPHOBIC/The Vegetation Engine/Hub/Terrain Shaders", false, 1006)]
        public static void ShowWindow()
        {
            window = GetWindow<TVETSHub>(false, "Terrain Shaders Module", true);
        ...
        void OnEnable()
        {
            ...
            searchFolders = AssetDatabase.FindAssets("Terrain Shaders");

            for (int i = 0; i < searchFolders.Length; i++)
            {
                if (AssetDatabase.GUIDToAssetPath(searchFolders[i]).EndsWith("Terrain Shaders.pdf"))
                {
                    assetFolder = AssetDatabase.GUIDToAssetPath(searchFolders[i]);
                    assetFolder = assetFolder.Replace("/Terrain Shaders.pdf", "");
                }
            }
            ...
            autorunPath = assetFolder + "/Core/Editor/TVETSHubAutoRun.cs";
            ...
            bannerText = "Terrain Shaders Module";
        ...
        void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("Welcome to the Terrain Shaders Module for the Vegetation Engine! Choose the render pipeline used in your project to make the shaders compatible with the current pipeline!", MessageType.Info, true);
            ...
            {
                EditorGUILayout.HelpBox("The Terrain Shaders Module is installed for the " + pipelineCurrent + "! Use the additional setting below to restart the render pipeline setup.", MessageType.Info, true);
        ...
        void DrawToolbar()
        {
            ...
            {
                EditorGUIUtility.PingObject(AssetDatabase.LoadAssetAtPath<Object>(assetFolder + "/Demo/Demo Terrain.unity"));
            }
            GUILayout.Space(-1);
        ...
        void SetDefineSymbols()
        {
            ...
            if (!defineSymbols.Contains("THE_VEGETATION_ENGINE_TERRAIN"))
            {
                defineSymbols += ";THE_VEGETATION_ENGINE_TERRAIN;";
