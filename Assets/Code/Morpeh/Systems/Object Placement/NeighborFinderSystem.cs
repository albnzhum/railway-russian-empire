...
namespace Railway.Morpeh
{
    ...
    public sealed class NeighborFinderSystem : UpdateSystem
    {
       ...
       private TerrainGridSystem tgs;
        ...
        {
            tgsFilter = World.Filter
                .With<TGSComponent>()
                .With<CellSelectionComponent>()
                .With<CellHighlightTag>().Build();
        ...
        {
            foreach (var itemDataEntity in itemDataFilter)
            {
                ref var itemData = ref itemDataEntity.GetComponent<ItemDataComponent>();
                
                foreach (var entity in tgsFilter)
                {
                    ref var tgsComponent = ref entity.GetComponent<CellSelectionComponent>();

                    tgsComponent.AvailableCells = GetAvailableCell(itemData.CurrentItem, tgsComponent.StartCell, tgsComponent.OccupiedCells).ToList();
                }
            }
        }
        
        public Cell[] GetAvailableCell(ShopItem currentItem, Cell startCell,
            Dictionary<ItemType, List<Cell>> occupiedCells)
        {
            Cell[] availableCells = new Cell[15];

            if (!occupiedCells.TryGetValue(currentItem.ItemType.Type, out var cells))
            {
                occupiedCells[currentItem.ItemType.Type] = new List<Cell>();
                cells = occupiedCells[currentItem.ItemType.Type];
            }
